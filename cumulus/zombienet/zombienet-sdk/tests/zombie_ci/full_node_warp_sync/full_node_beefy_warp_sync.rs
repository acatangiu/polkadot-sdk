...
//! Integration test for BEEFY-based warp sync.
//!
//! Starts a relaychain + parachain network where some relay chain nodes use `--sync beefy-warp`
//! to synchronise using BEEFY finality proofs instead of GRANDPA ones.
//!
//! The test verifies:
//! 1. The syncing node downloads BEEFY warp proofs and completes warp sync.
//! 2. The syncing node fills in historical block gaps (gap sync).
//! 3. Both the syncing node and existing nodes reach the expected best block height.
//! 4. Dynamically added nodes can also BEEFY warp sync successfully.
...
async fn assert_warp_sync(node: &NetworkNode) -> Result<(), anyhow::Error> {
	...
	log::info!("Asserting BEEFY warp sync for node {}", node.name());
	// We are interested only in Relaychain Warp sync (relaychain and parachain nodes),
	// thus exclude lines containing "[Parachain]"
	let result = node
		...
		.await?;
	if !result.success() {
		return Err(anyhow!("BEEFY warp sync is not started"));
	}
	let result = node
		.wait_log_line_count_with_timeout(
		...
		.await?;
	if !result.success() {
		return Err(anyhow!("BEEFY warp sync block import is not started"));
	}
	let result = node
		.wait_log_line_count_with_timeout(
		...
		.await?;
	if !result.success() {
		return Err(anyhow!("BEEFY warp sync block import is not progressing"));
	}
	...
	let result = node
		...
		.await?;
	if !result.success() {
		return Err(anyhow!("BEEFY warp sync is not complete"));
	}
...
async fn full_node_beefy_warp_sync() -> Result<(), anyhow::Error> {
